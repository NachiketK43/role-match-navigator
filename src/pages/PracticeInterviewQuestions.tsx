  ...
  const generateQuestions = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('generate-interview-questions', {
        body: {
          targetRole,
          targetCompany,
          jobDescription
        ...
        <div className="max-w-5xl mx-auto space-y-8">
          ...
          <div className="space-y-3">
            ...
            <p className="text-lg text-muted-foreground">
              Generate 10 AI-tailored interview questions specific to your target company's domain. Get coached answers with recommended formats and personalized preparation tips.
            </p>
          </div>

          {/* Rate Limit Banner */}
          {isRateLimited && <RateLimitBanner remainingTime={remainingTime} message={rateLimitInfo?.message} getRemainingTimeFormatted={getRemainingTimeFormatted} />}

          {/* Input Section */}
          {!questions && <Card className="p-8 shadow-card space-y-6">
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    ...
                    <Input id="targetRole" placeholder="e.g., Senior Software Engineer" value={targetRole} onChange={e => setTargetRole(e.target.value)} className="h-12" />
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <Input id="targetCompany" placeholder="e.g., Microsoft" value={targetCompany} onChange={e => setTargetCompany(e.target.value)} className="h-12" />
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <Textarea id="jobDescription" placeholder="Paste the complete job description here (optional - AI will infer based on role and company if not provided)..." className="min-h-[250px] resize-y" value={jobDescription} onChange={e => setJobDescription(e.target.value)} />
              ...
              <div className="flex justify-center pt-4">
                <Button size="lg" className="px-8 py-6 text-lg font-semibold shadow-elevated" onClick={generateQuestions} disabled={isGenerating || isRateLimited}>
                  {isGenerating ? <>
          ...
          {/* Results Section */}
          {questions && <div className="space-y-6 animate-fade-in">
              {/* Final Note Section */}
              {questions.finalNote && <Card className="p-6 bg-muted/50 border-2 border-primary/20 shadow-card">
              ...
              <Card className="p-8 shadow-card">
                <div className="space-y-4">
                  ...
                  return <AccordionItem key={index} value={`question-${index}`} className={`border-2 rounded-lg px-6 py-3 transition-all ${isPracticed ? 'opacity-60 border-primary/30 bg-muted/30' : 'hover:border-primary/50 hover:shadow-md'}`}>
                          <AccordionTrigger className="hover:no-underline py-3">
                            <div className="flex items-start gap-3 text-left w-full">
                              {isPracticed && <CheckCircle2 className="h-5 w-5 text-primary mt-1 flex-shrink-0" />}
                              <div className="space-y-2 flex-1">
                                <p className={`font-semibold text-base ${isPracticed ? 'line-through' : ''}`}>
                                  {q.question}
                                </p>
                                {isPracticed && <Badge variant="secondary" className="text-xs">
                                    ✓ Practiced
                                  </Badge>}
                              </div>
                            </div>
                          ...
                          <AccordionContent className="pt-4 space-y-4">
                            <div className="space-y-4">
                              ...
                              <div className="flex gap-2 pt-2 flex-wrap">
                                <Button size="sm" variant="outline" onClick={() => generateQuestions()} className="gap-2">
                                  <RefreshCw className="h-3 w-3" />
                                  Regenerate All Questions
                                </Button>
                                <Button size="sm" variant={isPracticed ? "secondary" : "default"} onClick={() => togglePracticed(index)} className="gap-2">
